...
class Player(SpriteManager): #enhartes from the spritemanger
    ...
    def jumpAndGravity(self, isCollidingWithGround, jumpLimit, initialLowHeight): #Need timer for when this instance goes....
        if self.isJumping:
            ...
            if not yJumpCheck(image) and not topRightCheck(image) or not yJumpCheck(image) and not topLeftCheck(image):
                self.isJumping = False
                self.falling = True
...
def checkRadius(image, block): #change this to the chunk method.
    ...
    if chunkWorld[whichChunkRender][0][0].y <= 0:
        sizex = 300
        distancex = abs(image.x - block.x)

        if distancex >= sizex and block.x >= 0 and block.x <= 1280 and block.y >= 350 and block.y <= 720 and block.tag == 'Bedrock':
            block.displayImage()

        if distancex <= sizex and block.y >= 350 and block.y <= 720:
            block.displayImage()
            return True

    else:
        if block.x >= -32 and block.x <= 1278 and block.y >= 350 and block.y <= 720:
            block.displayImage()
            return True
        elif chunkWorld[whichChunkRender][0][0].y <= 0:
            background.y = 401
        else:
            background.y = 0 
...
def foreheadCheck(image, right, left, i):
    if right:
        canMove = True
        change = False
        for block in chunkWorld[whichChunkRender][0]:
...
def topRightCheck(image):
    canMove = True
    change = False
    for block in chunkWorld[whichChunkRender][0]:
        if image.y >= block.y and image.y <= block.y + 32 and block.x >= image.x + 32 and image.x + 55 <= block.x + 32:
            canMove = False
            change = True 
        if change == False:
            canMove = True
...
def topLeftCheck(image):
    canMove = True
    change = False
    for block in chunkWorld[whichChunkRender][0]:
        if image.y >= block.y and image.y <= block.y + 32 and image.x >= block.x and image.x <= block.x + 32:
            canMove = False
            change = True
        if not change:
            canMove = True
    ...
    for chunk in range(numberChunks): #-The chunk index.
        ...
        for v in range(verticalLength):
            ...
            for h in range(25):
                ...
                elif v == verticalLength - 1:
                    chunkWorld[chunk][0].append(Blocks('Bedrock', 'Sprites/BedRock.png',currentx, currenty))
                else:
                    #adds a block to the block list in the chunk list.
...
def manageChunkWorld(blockCollided):
    ...
    for i in range(len(chunkWorld) - 1, - 1, -1): #Runs through the length of the chunk world list.
        ...
        if image.x >= chunkWorld[i][1] and image.x <= chunkWorld[i][1] + chunkWorld[i][2]:
            ...
            canMoveRight = foreheadCheck(image, True, False, i)
            canMoveLeft = foreheadCheck(image, False, True, i)
            for block in chunkWorld[i][0]:
...
#Medium is 750x50
#Small is 375x50
createChunkWorld((3000, 60))
#World lengths notes: 2000 by 50 good, 4000 by 50 good, 6000 by 50 good. 50,000 by 50 good! #Widths size does not affect peformentss
#Only deepness
dieCheck = False
